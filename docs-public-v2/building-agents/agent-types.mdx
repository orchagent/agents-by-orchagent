...
Code-based agents run your code in **E2B sandboxes** - secure, ephemeral execution environments. Each call spins up a fresh sandbox, runs your script, and returns the result. You have full control over:
...
Create a simple Python script that reads from stdin and writes to stdout:

```python
# main.py - Code agent that analyzes a repository
import json
import sys
import subprocess
...
Code agents communicate via stdin/stdout:

**Input (stdin):** JSON object with your input data:
...
**Output (stdout):** JSON object with your result:
...
**File Input:** When files are uploaded, you receive a manifest:
...
Your code reads files from the `path` locations and processes them.
...
my-code-agent/
├── orchagent.json      # Agent configuration
├── main.py             # Entry point (or index.js for Node.js)
├── requirements.txt    # Python dependencies (or package.json)
└── README.md           # Documentation (optional)
```

The CLI auto-detects your entrypoint. Detection order: `main.py`, `app.py`, `index.py`, `main.js`, `index.js`. You can override this in your manifest:
...
**1. Install locally** - Use with any AI coding tool (Claude Code, Cursor, Codex, etc.):
...
This writes the skill to multiple directories:
- `.claude/skills/` (Claude Code)
- `.cursor/skills/` (Cursor)
- `.codex/skills/` (Codex)
- `.agent/skills/` (Universal)

**2. Compose with OrchAgent agents** - Inject skill knowledge into an agent call:
...
Agent authors can define `default_skills` in their manifest - these are automatically injected when the agent runs.
