...
| I want to... | Use | Why |
|---|---|---|
| Analyze sentiment or classify text | Direct LLM | One LLM call, structured JSON output |
| Summarize or translate documents | Direct LLM | One LLM call, no tools needed |
| Generate marketing copy or emails | Direct LLM | Prompt engineering, structured output |
| Extract data from text (names, dates, etc.) | Direct LLM | One LLM call with output schema |
| Fix code until tests pass | Managed loop | LLM reads code, runs tests, iterates |
| Review pull requests or audit code | Managed loop | LLM navigates files, checks patterns |
| Research a topic and write a report | Managed loop | LLM searches, reads, synthesizes |
| Scan repos for secrets or vulnerabilities | Code runtime | Deterministic logic, fast, no LLM needed |
| Process uploaded files (PDF, CSV, images) | Code runtime | File I/O, custom parsing logic |
| Call external APIs and transform data | Code runtime | Full HTTP control, auth, error handling |
| Run a multi-model pipeline (different LLMs per step) | Code runtime | You control which LLM handles each step |
| Build a Discord bot or webhook listener | Code runtime | Persistent service, event-driven |
...
<Info>
**Still unsure?** Start with a direct LLM agent. If you find yourself thinking "I wish it could run a command" or "it needs to iterate," upgrade to a managed loop. If you need full control, switch to code runtime. You can always change later — the agent type is just a manifest declaration.
</Info>
...
| | **Direct LLM** | **Managed Loop** | **Code Runtime** | **Skill** |
|---|---|---|---|---|
...
| **Providers** | Any (OpenAI, Anthropic, Gemini) | Anthropic only (for now) | Any (you call the API) | N/A |
