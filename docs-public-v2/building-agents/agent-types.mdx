...
| | Prompt Agent | Agentic Agent | Code Agent | Skill |
|-|--------------|---------------|------------|-------|
...
I want to build an agent
  │
  ├─ Does it need to run commands or work with files in a sandbox?
  │   │
  │   ├─ No → Prompt agent
  ...
  │   └─ Yes
  │       │
  │       ├─ Do I need full programmatic control or non-LLM logic?
  │       │   │
  │       │   ├─ No → Agentic agent
  │       │   │   "Give it a prompt + tools, platform runs the loop"
  │       │   │
  │       │   └─ Yes → Code agent
  │       │       "You write main.py, you control everything"
  │       │
  │
  └─ I don't want to execute tasks, I want to share knowledge
      └─ Skill
          "Markdown instructions that enhance any agent"
```

<Tip>
**Start with the simplest type that works.** Most agents should be prompt agents. If you need tools, try agentic first. Only reach for code agents when you need full control.
</Tip>
...
- You'd otherwise write a code agent just to orchestrate LLM + subprocess calls

**What you provide:**
```
my-agentic-agent/
├── orchagent.json      # Agent manifest (type: "agentic", custom_tools)
...
Without agentic agents, this same agent required **200+ lines of Python** handling the Claude API calls, retry loops, file scaffolding, error analysis, and file merging. With agentic agents, it's just a prompt and a few tool definitions.
...
Every agentic agent automatically gets these tools:
...
Agentic agents currently support **Anthropic only** (Claude). This is because Anthropic has the best tool-use support. More providers will be added in the future.

---

## Code Agents

Code agents run your Python or JavaScript in **E2B sandboxes** — secure, isolated environments. Each call spins up a fresh sandbox, runs your script, and returns the result. You have full control over everything.

**When to use:**
- You need full programmatic control over the execution flow
- Your agent doesn't use an LLM at all (pure data processing, file conversion, etc.)
...
- Agentic agents don't give you enough control
...
<Note>
Agentic agents currently only support `"anthropic"`. This will be expanded in the future.
</Note>
...
| Create a **Skill** when... | Create a **Prompt Agent** when... | Create an **Agentic Agent** when... | Create a **Code Agent** when... |
|---------------------------|----------------------------------|------------------------------------|---------------------------------|
...
| Content is reusable across contexts | No sandbox or file access needed | You'd otherwise write boilerplate orchestration | Your agent doesn't use an LLM |
