...
| `manifest.orchestration_mode` | `"strict"` (default) or `"flexible"`. Strict removes `bash` and requires dependency calls. See [Strict vs Flexible Mode](/building-agents/orchestration#strict-vs-flexible-mode). |
...
<Note>
**Strict mode is the default.** When you publish an orchestrator with dependencies, `orchestration_mode` defaults to `"strict"` — bash is disabled and the agent must use its custom tools. If your orchestrator needs bash access, explicitly set `"orchestration_mode": "flexible"` in your manifest.
</Note>
...
<Tip>
Mocked tests are ideal for CI/CD — they verify the LLM's reasoning without requiring deployed sub-agents. See the [CI/CD Guide](/guides/cicd) for GitHub Actions setup.
</Tip>
