...
|---|---|---|
| **How it works** | LLM decides when to call dependencies via tool use | Your Python code calls dependencies directly |
| **You write** | `orchagent.json` + `prompt.md` (no code) | `orchagent.json` + Python code |
...
| `manifest.per_call_downstream_cap` | Cost cap per downstream dependency call |
...
A code runtime orchestrator runs your Python code directly. You control the execution flow — which agents to call, in what order, and how to process results. Use this when you need deterministic logic, custom data transformation, or parallel fan-out.
...
|-------|-----------------|
| Sandbox startup | ~5s |
| SDK installation (if `orchagent-sdk` in requirements.txt) | ~55s |
...
Wrap SDK calls in try/except and log the error:
