...
| `orchestration_mode` | No | `"strict"` or `"flexible"`. Controls whether the agent must use dependency-path execution. See [Orchestration Mode](#orchestration-mode) below. Defaults to `"strict"` for new publishes with dependencies. |
...
| Value | Behavior |
|-------|----------|
| `"strict"` | `bash` is removed from available tools. The agent must call at least one dependency before submitting results. This is the **default for new publishes** with dependencies. |
...
- New publishes with dependencies default to `"strict"` unless you explicitly set `"flexible"`.
