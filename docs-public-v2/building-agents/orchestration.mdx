...
OrchAgent supports two types of composition:

1. **Skill composition** - Inject knowledge/instructions into agent prompts
2. **Agent composition** - Call other agents from within your agent

## Skill Composition

Skills are passive knowledge that can be injected into agents. Unlike agent-to-agent calls, skill composition happens at the prompt level - skill content is prepended to the agent's prompt before the LLM call.
...
Callers can modify skill behavior:

```bash
# Add more skills to agent's defaults
orch call yourorg/code-reviewer --skills yourorg/security-rules

# Replace default_skills entirely
orch call yourorg/code-reviewer --skills-only yourorg/security-rules
...
| **Can be overridden** | Yes (--skills flags) | No (fixed in manifest) |
...
Orchestrator agents call other agents to build complex workflows. This section covers how to create agents that compose multiple agents together.
