...
Skills are passive knowledge that can be injected into agents. Skills work for both prompt and code agents:

- **For prompt agents:** skill content is prepended to the agent's prompt before the LLM call
- **For code agents:** skills are mounted as files at `$ORCHAGENT_SKILLS_DIR` (see [Agent Types](/building-agents/agent-types#skills-in-code-agents))
...
- **Server (`orch call`)**: Caller override flags (`--skills`, `--skills-only`, `--no-skills`) are silently ignored
- **Local (`orch run`)**: CLI shows a warning and asks for confirmation before overriding
