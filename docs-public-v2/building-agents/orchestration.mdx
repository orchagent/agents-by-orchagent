...
Skills are passive knowledge that can be injected into agents. Skills work for both prompt and tool agents:

- **For prompt agents:** skill content is prepended to the agent's prompt before the LLM call
- **For tool agents:** skills are mounted as files at `$ORCHAGENT_SKILLS_DIR` (see [Agent Types](/building-agents/agent-types#skills-in-tools))
