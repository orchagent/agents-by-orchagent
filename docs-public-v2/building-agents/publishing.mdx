...
# Create agentic agent (LLM with tools in sandbox)
orch init my-agent --type agentic

# Create code-based agent (full control)
orch init my-agent --type code

cd my-agent
```

This creates a `my-agent/` directory with the basic structure. Prompt and agentic agents get `orchagent.json`, `prompt.md`, and `schema.json`. Code agents get `orchagent.json` and a `main.py` template.
...
For **code agents**, the platform also:
