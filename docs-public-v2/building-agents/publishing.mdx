...
For tool or agent type agents that need to run continuously (Discord bots, webhook listeners, background workers), deploy as an always-on service:
...
**Source-available agents** expose the full prompt and code. Users can inspect and run locally. Use this for open/community agents where transparency builds trust.
