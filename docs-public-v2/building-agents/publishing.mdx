...
Paid agents are **server-only for non-owners** — other users must use `orch call` (not `orch run`). You can still download your own paid agents for testing.
...
| **Server Only** (default) | No — redacted | No — use `orch call` | No — blocked | "Server Only" |

**Server-only agents** (the default) redact the prompt, manifest, and skill files from public API responses. This protects your intellectual property while still letting users run the agent via `orch call`.
...
orch call yourorg/my-agent

# Call a specific version
orch call yourorg/my-agent@v1
orch call yourorg/my-agent@v2
```
...
| `--price <amount>` | Set per-call price in USD (e.g., `0.50` for $0.50). Requires seller onboarding. |
| `--local-download` | Allow users to download and run locally. Without this flag, agents default to server-only. |

## Distribution

By default, new agents are **server-only** — users run them via `orch call` on orchagent's servers. This is the recommended default for most agents.

To allow users to download and run your agent locally with `orch run`:
