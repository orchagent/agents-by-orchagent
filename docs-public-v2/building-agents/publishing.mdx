...
Create `.orchagentignore` for custom exclusions (same syntax as `.gitignore`).

### Size Limits

- Maximum bundle size: **25MB**
- Use `orch publish --dry-run` to preview what will be bundled
...
Code agent bundles have a 25MB size limit. To reduce size:
- Check for large files that shouldn't be included
- Remove `node_modules/` or `venv/` directories
- Use `.orchagentignore` to exclude files

### "Entrypoint not found"

The CLI auto-detects entrypoints in this order: `main.py`, `app.py`, `index.py`, `main.js`, `index.js`. If your entrypoint has a different name:
