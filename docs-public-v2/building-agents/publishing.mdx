...
This creates a directory with the basic structure: `orchagent.json`, `prompt.md`, and `schema.json`. For code runtime agents with `--language javascript`, it creates `main.js` and `package.json` instead. The manifest uses `"type": "agent"` with canonical fields.
...
4. **Dependency check** — verifies all dependencies exist (for orchestrators)
5. **Cycle detection** — ensures no circular dependencies
