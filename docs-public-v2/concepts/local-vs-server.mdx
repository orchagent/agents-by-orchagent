...
<Note>
**Paid agents** are server-only for non-owners. If an agent has a per-call price, other users must use `orch run` (cloud) — `orch run --local` will return an error. Authors can always download and run their own paid agents locally. See [Monetization](/concepts/monetization).
</Note>
