...
Tool and agent type agents have separate limits for compute time (sandbox execution):
...
<Note>
Sandbox agent limits are **separate** from prompt agent limits. You can use both types with their respective quotas.
</Note>
...
[Always-on services](/using-agents/services) have separate limits from on-demand agent runs:
...
Service compute time is metered by runtime minutes and counts toward your workspace usage. See [Billing](/concepts/billing#service-usage) for details.
