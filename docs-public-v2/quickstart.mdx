...
<Tip>
orchagent hosts both **agents** (active executors) and **skills** (passive knowledge). The commands below work for both. See [Agents & Skills](/building-agents/agent-types) for the difference.
</Tip>
...
This creates:
- `orchagent.json` — agent manifest (type: `"agent"`)
...
<Tip>
**Building a code runtime agent?** Use `--language javascript` to scaffold a JavaScript project with `main.js` and `package.json`, or use `--template discord-js` for a JavaScript Discord bot. See [CLI Commands](/using-agents/cli-commands#init) for all options.
</Tip>
...
<Tip>
By default, `orch init` creates a direct LLM agent (prompt + schema). Use `orch init my-agent --type agent` with `--run-mode` to customize behavior. See [CLI Commands](/using-agents/cli-commands#init) for all options.
</Tip>
