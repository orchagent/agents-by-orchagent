...
## Tool Agent Input Contract

When files are uploaded to a tool agent, the agent receives a JSON manifest via stdin:
...
Your tool agent reads the files from their `path` locations and processes them.
...
| `POST /agents/{id}/upload` | Upload code bundle (tool agents only) |
...
| Endpoint | Description |
|----------|-------------|
| `GET /billing/balance` | Get your prepaid credit balance |
...
| `GET /billing/earnings` | Get seller earnings breakdown |

### Sellers

| Endpoint | Description |
|----------|-------------|
| `POST /sellers/onboard` | Start Stripe Connect onboarding |
| `GET /sellers/status` | Get seller verification and payout status |
| `POST /sellers/dashboard-link` | Get Stripe Express Dashboard login URL |
