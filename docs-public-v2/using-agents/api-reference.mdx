...
| `PATCH /workspaces/{id}/schedules/{schedule_id}` | Update schedule (cron, timezone, enable/disable) |
...
| Endpoint | Description |
|----------|-------------|
| `POST /webhooks/{schedule_id}/{webhook_secret}` | Trigger agent via webhook |
