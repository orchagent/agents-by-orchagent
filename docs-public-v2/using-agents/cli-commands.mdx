...
| `orch schema <agent>` | Dump input/output schemas as JSON | - |
| `orch context` | Dump full CLI context for AI agents (YAML frontmatter + markdown) | - |
| `orch describe <command>` | Show structured contract for one command | - |
...
Managing agents?
  ...
  |-- Inspect details        --> orch info / orch schema / orch tree
  \-- Move between workspaces --> orch transfer

Need machine-readable CLI metadata?
  |-- Full command index     --> orch context
  \-- Single command schema  --> orch describe
...
orch agents --fields name,version,type

# Paginate results
orch agents --limit 10 --offset 20
```
...
| `--fields <fields>` | Comma-separated fields to include in JSON output (implies `--json`) |
...
# Return only specific fields (implies --json)
orch info joe/code-reviewer --fields name,type,input_schema
```
...
Dump agent input/output schemas as machine-readable JSON. Designed for programmatic consumption by AI agents and automation tools.

```bash
# Show both input and output schemas
orch schema joe/code-reviewer

# Input schema only
orch schema joe/code-reviewer --input-only

# Output schema only
orch schema joe/code-reviewer --output-only

# Full agent spec (type, callable, providers, secrets, dependencies, custom_tools)
orch schema joe/code-reviewer@v2 --full
```
...
| Flag | Description |
|------|-------------|
| `--input-only` | Show only the input schema |
| `--output-only` | Show only the output schema |
| `--full` | Show full agent spec including schemas, secrets, dependencies, and custom tools |

Output is always JSON. Default mode returns `{ agent, type, input_schema, output_schema }`. Use `--full` for the complete agent specification.

### context

Dump a machine-readable guide for the entire CLI. Designed for AI agents and automation tools to discover commands without parsing `--help` text.

```bash
# Dump full CLI context
orch context
```

Output format:

- YAML frontmatter with CLI name, version, and all top-level commands
- Per-command metadata: description, flags, mutation signal, and dry-run support
- Markdown guide with auth, output conventions, and command usage patterns

This command is intended to be consumed directly by agents that need a complete CLI capability map.

### describe

Show a structured contract for a single command or subcommand.
...
# Alternate path syntax also works
orch describe schedule:create --json

# Human-readable format
orch describe run --markdown
```
...
- `arguments[]` with required/optional and format hints
- `flags[]` with type, descriptions, aliases, and `value_required` when a value is required
- `mutations` and `dry_run` signals
- `examples`, `aliases`, and nested `subcommands` metadata (for groups)

If the command path is invalid, `orch describe` returns a structured not-found error and suggests `orch context`.
...
# Return only specific fields (implies --json)
orch secrets list --fields name,secret_type
...
View execution logs. Use with no arguments to list recent runs, an agent name to filter, or a run ID for full detail. When filtering by agent name, always-on service logs are automatically included if the agent has an active service.

```bash
# List recent runs
orch logs

# Filter by agent name (also shows always-on service logs if deployed)
orch logs scanner

# View live logs from an always-on service (skips run history)
orch logs scanner --live
...
orch logs --limit 10 --offset 20

# JSON output
orch logs --json

# Return only specific fields (implies --json)
orch logs --fields status,duration_ms,agent_name
```
...
| `--live` | Show live logs from always-on service only (skips run history) |
| `--json` | Output as JSON |
| `--fields <fields>` | Comma-separated fields to include in JSON output (implies `--json`) |
...
| Target | Behavior |
|--------|----------|
| *(none)* or agent name | Lists recent runs in a table. If filtering by agent name and an always-on service exists, service logs are shown below the runs table. If no runs are found, suggests active services in the workspace with `--live` commands. |
| Agent name with `--live` | Shows only the always-on service logs (no run history) |
...
| `ORCHAGENT_OUTPUT` | Output format: `json` or `text` (overrides TTY auto-detection) |

**Resolution order:** Command-line flags > Environment variables > Config file > Defaults

### TTY Auto-Detection

When stdout is not a TTY (piped, redirected, or called from an AI agent), the CLI automatically switches to JSON output and disables spinners on all commands that support `--json`. This means piping and redirection work without needing the `--json` flag:

```bash
# These automatically output JSON (non-TTY detected)
orch agents | jq '.[].name'
orch run joe/scanner --data '{"url": "..."}' > result.json
RESULT=$(orch info joe/my-agent)

# In a TTY terminal, you still get human-readable tables and spinners
orch agents
```
...
| Scenario | Output |
|----------|--------|
| TTY terminal (default) | Human-readable tables, colors, spinners |
| Piped / redirected (non-TTY) | JSON, no spinners |
| `--json` flag (any context) | JSON (explicit) |
| `ORCHAGENT_OUTPUT=json` | Always JSON, even in TTY |
| `ORCHAGENT_OUTPUT=text` | Always human-readable, even in non-TTY |
...
| `--json` | Output as JSON |
| `--fields <fields>` | Comma-separated fields to include in JSON output (implies `--json`) |
| `--limit <n>` | Maximum number of schedules to return (default: 100) |
| `--offset <n>` | Number of schedules to skip |
...
# Paginate
orch schedule runs <schedule-id> --limit 10 --offset 20

# Return only specific fields (implies --json)
orch schedule runs <schedule-id> --fields status,duration_ms,started_at
```
...
| `--json` | Output as JSON |
| `--fields <fields>` | Comma-separated fields to include in JSON output (implies `--json`) |
| `--limit <n>` | Maximum number of services to return (default: 100) |
| `--offset <n>` | Number of services to skip |
...
# Process JSON output (--json is auto-enabled when piped)
orch run acme/scanner doc.pdf | jq '.data.total'

# Handle structured errors (errors are JSON when --json is active)
...
if echo "$output" | jq -e '.error' > /dev/null 2>&1; then
  ...
  echo "Failed with $code: $(echo "$output" | jq -r '.message')"
fi
...
<Tip>
When `--json` is active, errors are returned as structured JSON on stdout with `error`, `code`, `message`, `hint`, and `exit_code` fields. See [Troubleshooting → Error Response Format](/troubleshooting#error-response-format) for the full schema.
</Tip>
