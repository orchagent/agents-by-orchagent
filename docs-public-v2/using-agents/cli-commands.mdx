...
<Note>
Only `tool` and `agent` type agents are eligible for service deployment. Sensitive environment variables (e.g., API keys, tokens) are rejected by `--env` and must be passed using `--secret` instead.
</Note>
