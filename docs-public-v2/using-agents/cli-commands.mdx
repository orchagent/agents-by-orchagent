...
| `orchagent workspace list` | List your workspaces |
| `orchagent workspace create <name>` | Create a team workspace |
| `orchagent workspace use <slug>` | Set default workspace |
| `orchagent workspace members` | List workspace members and invites |
| `orchagent workspace invite <email>` | Invite member to workspace |
| `orchagent workspace leave` | Leave a workspace |
...
Workspaces let you organize team access, share agents privately, and manage billing. Every user has a personal workspace, and can create or join team workspaces.
...
```bash
# List workspaces
orchagent workspace list

# JSON output for scripting
orchagent workspace list --json
```
...
```bash
# Create a workspace (slug auto-generated from name)
orchagent workspace create "My Team"

# Create with custom slug
orchagent workspace create "My Team" --slug my-team
```

<Tip>
The workspace slug is used in agent names and URLs. Choose something short and memorable.
</Tip>

### workspace use

Set the default workspace for CLI operations. This affects where agents are published and which workspace context is used.

```bash
# Set default workspace
orchagent workspace use my-team
...
List all members and pending invites in a workspace.

```bash
# List members of current workspace
orchagent workspace members

# List members of specific workspace
orchagent workspace members my-team

# JSON output for scripting
orchagent workspace members --json
```
...
orchagent workspace invite alice@example.com

# Invite as owner
orchagent workspace invite alice@example.com --role owner

# Invite to specific workspace
orchagent workspace invite alice@example.com --workspace my-team
```

**Available roles:**
- `member` - Can use and publish agents
- `owner` - Can manage members, invites, and workspace settings
...
```bash
# Leave current workspace (prompts for confirmation)
orchagent workspace leave

# Leave specific workspace
orchagent workspace leave my-team
```

<Warning>
You cannot leave your personal workspace. If you're the last owner of a team workspace, you must assign another owner before leaving.
</Warning>
...
<Note>
**Locked skills:** If an agent has `skills_locked: true`, your override flags are ignored on server calls (`orch call`). For local runs (`orch run`), the CLI will warn you and ask for confirmation before overriding.
</Note>
...
orchagent delete my-agent --dry-run
...
orchagent publish --skills yourorg/style@v1,yourorg/rules@v1

# Lock skills (callers cannot override)
orchagent publish --skills yourorg/owasp@v1 --skills-locked
```

**The `--dry-run` flag** runs all validation and shows exactly what would be published without making any changes:
- Validates manifest, prompt, and schema files
- Shows the version that would be assigned (e.g., v1, v2)
- For code agents: shows bundle preview (file count, size, entrypoint)
- Displays the API endpoint that would be created
