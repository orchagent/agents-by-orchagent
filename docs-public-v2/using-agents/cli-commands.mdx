...
```bash
# Basic call with file (code agents: uploaded to sandbox)
...
<Tip>
Use `--mine` to see all your published agents, including private ones that don't appear in public search results.
</Tip>
...
# Create a code-based agent in a new directory
orch init my-agent --type code
...
Monetize your agents by setting per-call prices. Requires Stripe Connect onboarding.
