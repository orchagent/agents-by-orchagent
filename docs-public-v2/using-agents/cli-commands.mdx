...
Validate your agent's configuration without publishing or running tests. Quick check to catch config issues before publishing.

```bash
# Validate agent in current directory
orch validate

# Validate with server-side checks (requires auth)
...
orch validate --profile myprofile
```

**What it checks:**
- `orchagent.json` structure and required fields
- `prompt.md` exists for prompt/agent types
- `schema.json` format (if present)
- Type and execution engine configuration
- Dependencies are published and callable (with `--server`)

**Use cases:**
- Quick validation before publishing
- CI/CD pipelines to catch config issues early
- Checking if all required files are in place
