...
| `orchagent run <agent>` | Download and run an agent or skill locally |
...
| `orchagent skill list` | List public skills |
| `orchagent skill install <skill>` | Install skill to local AI tool directories |
| `orchagent search <query>` | Search public agents and skills |
...
# Filter by name
orchagent agents --filter "invoice"

# JSON output
orchagent agents --json
```
...
```bash
orchagent search "invoice" --sort stars
...
orchagent skill list

# Filter by name
orchagent skill list --filter "react"

# JSON output
orchagent skill list --json
```
