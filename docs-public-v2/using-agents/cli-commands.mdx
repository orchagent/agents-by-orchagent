...
| `orch run <agent>` | Download and run an agent locally |
| `orch init` | Initialize agent directory structure |
| `orch publish` | Publish agent from local files |
| `orch agents` | List public agents |
| `orch search <query>` | Search public agents |
| `orch star <agent>` | Star or unstar an agent |
| `orch fork <agent>` | Fork a public agent |
...
Publish an agent from the current directory.
