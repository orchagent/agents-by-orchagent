...
| `orch skill install <skill>` | Install skill to local AI tool directories |
...
# Files with metadata
orch call acme/invoice-scanner --file invoice.pdf --metadata '{"format":"detailed"}'
...
orch publish --entrypoint custom.py
```
...
For code-based agents, `orch publish` automatically:
