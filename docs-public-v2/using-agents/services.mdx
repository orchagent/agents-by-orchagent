...
When you filter by agent name, `orch logs` automatically detects if the agent has an always-on service and shows its live logs alongside the run history.
...
Your HTTP server should bind to port 3000. See [HTTP Ingress](#http-ingress-receiving-webhooks) for details.
