...
```bash
# Deploy a service
orch service deploy acme/discord-bot --secret DISCORD_BOT_TOKEN
...
<Note>
Only **tool** and **agent** type agents with a published code bundle are eligible for service deployment. Prompt-only agents cannot be deployed as services.
</Note>
...
1. You publish a tool or agent type agent with code (`orch publish`)
...
7. Workspace secrets are injected as environment variables into the container
...
Use the `--secret` flag to inject workspace secrets as environment variables. Secrets are resolved by name at deploy time and injected securely into the container.
...
<Tip>
Configure workspace secrets in the [dashboard](https://orchagent.io/dashboard) under your workspace settings before deploying a service that depends on them.
</Tip>
...
```json
{
  "name": "my-agent",
  "type": "tool",
  "run_command": "python src/start.py"
}
```
</Tip>
...
# Deploy as a service with the bot token
...
```bash
# Deploy a webhook listener that processes incoming events
...
```bash
# Deploy a worker that polls a queue
...
- Verify your agent has a published code bundle (prompt-only agents cannot be deployed as services)
...
- Ensure all required secrets are configured in your workspace
