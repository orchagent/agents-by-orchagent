...
Scheduled runs use the LLM credentials stored in your workspace:

1. If the schedule has a `--provider` set, that provider's key is used
2. Otherwise, the agent's default provider is used
3. Keys must be configured in your workspace before creating the schedule

<Tip>
Store your LLM API keys in the [dashboard](https://orchagent.io/dashboard) under your workspace settings, or use `orch llm-config` to configure them via CLI.
</Tip>
